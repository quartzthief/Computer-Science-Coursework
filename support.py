...
def import_folder(*path):
	frames = []
	#walks through the folder specified and gets all the content
...
def character_importer(cols, rows, *path):
	...
	for row, direction in enumerate(('down', 'left', 'right', 'up')):
		# also for importing walking animationone
		new_dict[direction] = [frame_dict[(col,row)]for col in range(cols)]
		#to get idle anmation contraption called an f string value has to be a list as it simplifies animation logic
		new_dict[f'{direction}_idle'] = [frame_dict[(0,row)]]
	return new_dict

# imports all charcter sprites and animations
def all_character_import(*path):
	new_dict = {}
	# underscores to show not caring for data values
	for _, __, image_names in walk(join(*path)):
		for image in image_names:
			#cuts string off when encounter a . (like in .png)
